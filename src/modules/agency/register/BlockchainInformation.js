...
export default class Step2 extends Component {
	...
	_validationErrors(val) {
		const errMsgs = {
			ethAddressValMsg: val.ethAddressVal ? '' : 'Valid Ethereum Address is required'
		};
		return errMsgs;
	}
							...
							<div className="form-group content form-block-holder">
								...
								<InputGroup>
									...
									<InputGroupAddon addonType="append">
										<Button className="btn btn-warning" onClick={this.handleLoadAddressClick}>
											Get metamask account
										</Button>
									</InputGroupAddon>
								</InputGroup>
								<div className={notValidClasses.nameValGrpCls}>{this.state.ethAddressValMsg}</div>
							</div>
						</form>
					</div>
				</div>
